...
if($arResult['ENABLE_TOOLBAR'])
{
	...
	if($arResult['ADD_EVENT_NAME'] !== '')
	{
		...
		{
			$optSite = \Bitrix\Main\Config\Option::get("sale", "SHOP_SITE_".$site['LID'], "");
		...
		foreach($sites as  $lid => $name)
		{
			...
			if (isset($arParams['INTERNAL_FILTER']['COMPANY_ID'])) {
				$dopFields['company_id'] = $arParams['INTERNAL_FILTER']['COMPANY_ID'];
			}

			if (isset($arParams['INTERNAL_FILTER']['ASSOCIATED_CONTACT_ID'])) {
				$dopFields['contact_id'] = $arParams['INTERNAL_FILTER']['ASSOCIATED_CONTACT_ID'];
			}
		...
		$addButton['LINK'] = 'javascript:void(0)';
        $addButton['ONCLICK'] = 'custom_create()';
