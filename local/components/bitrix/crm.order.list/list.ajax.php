...
require_once ($_SERVER['DOCUMENT_ROOT'].'/local/php_interface/api/classes/RtopFailureReason.php');
...
elseif ($action === 'SAVE_PROGRESS')
{
	...
	$customFailure = isset($_REQUEST['CUSTOM_FAILURE']) ? $_REQUEST['CUSTOM_FAILURE'] : '';
	...
	{
		$failure = RtopFailureReasonTable::getList(['filter' => ['ORDER_ID' => $ID]])->fetch();
		if(empty($failure))
			RtopFailureReasonTable::add(['ORDER_ID' => $ID, 'FAILURE_ID' => $customFailure]);
		else
			RtopFailureReasonTable::update($failure['ID'], ['FAILURE_ID' => $customFailure]);
	}
