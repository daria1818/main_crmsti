BX.ready(function(){
	...
	if(overflow){
		BX.bind(overflow, "click", BX.delegate(function(e){
			toggleSelects(document.querySelector('.r_multi_field--select.active'), false, false);
			toggleSelects(document.querySelector('.r_search_field--search.active'), false, false);
			toggleSelects(document.querySelector('.r_select_field--select.active'), false, false);
		}));
	}
				...
				onsuccess: function(result){
					if(result.length == 0){
						...
						resultBlock.append(BX.create('A', {
							...
							events: {
								click: BX.delegate(function(){
									tableToExcel('report_table', 'Отчет о потерянных клиентах', 'report.xls');
								}, this)
							}
						}), table);
						table.append(
								...
								children: [
									BX.create('TR', {
										children: [
											...
											BX.create('TD', {
												text: 'Дата и сумма последнего заказа/сделки'
											}),
											BX.create('TD', {
												text: 'Регион'
											})
						...
						for(let row of Object.keys(result)){
							...
							newCell.innerHTML = "<a target='_blank' href='"+document.location.origin+"/crm/contact/details/"+result[row].CLIENT_ID+"/'>" + result[row].NAME + '</a>' + ( result[row].COMPANY_TITLE != null ? " (" + result[row].COMPANY_TITLE + ")" : "");

							newCell = newRow.insertCell(1);

							if(result[row].END_ORDER != null)
								newCell.innerHTML = "<a target='_blank' href='"+document.location.origin+result[row].END_ORDER.LINK+"'>Заказ №" + result[row].END_ORDER.ID + "</a><br/>Дата: " + result[row].END_ORDER.DATE + "<br/>Сумма: " + result[row].END_ORDER.SUM +"<br/>";
							if(result[row].END_DEAL != null)
								newCell.innerHTML = "<a target='_blank' href='"+document.location.origin+result[row].END_DEAL.LINK+"'>Сделка №" + result[row].END_DEAL.ID + "</a><br/>Дата: " + result[row].END_DEAL.DATE + "<br/>Сумма: " + result[row].END_DEAL.SUM +"<br/>";
