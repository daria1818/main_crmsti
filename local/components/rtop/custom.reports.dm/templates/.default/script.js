BX.ready(function() {
	...
	$('body').on('focus', 'input[name=SECTION_ID]', function () {
		$('input[name=SECTION_ID]').attr('id', 'SECTION_ID');
        window.open(
			'/bitrix/admin/iblock_section_search.php?lang=ru&IBLOCK_ID=95&simplename=Y&n=SECTION_ID', 
			'', 
    ...
    $('body').on('focus', 'input[name=PRODUCT_ID]', function () {
    	BX.Crm.Order.Product.listObj_crm_order_product_list.addProductSearch({lang: 'ru', siteId: 'dm', orderId: '0'});
    })
    ...
    BX.addCustomEvent("onWindowClose", function(e){
    	let element = document.getElementById("CCustomReportsDmComponent_search_container");
    	if(!BX.hasClass(element, "main-ui-filter-search--showed"))
    		element.click();
    })

    function displayDeleteFields(){
    	let inputs = document.querySelectorAll('input[name=PRODUCT_IDS], input[name=SECTION_ID]');
    	if(inputs.length > 0){
