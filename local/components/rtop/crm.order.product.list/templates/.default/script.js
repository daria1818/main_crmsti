...
if(typeof BX.Crm.Order.Product.List === "undefined")
{
	...
	BX.Crm.Order.Product.List.prototype =
	{
		...
		onProductAdd: function(params, iBlockId)
		{
			...
			{
				this.setChanged();
				let productId = document.querySelector('input[name="PRODUCT_ID"]');
				if(productId){
					productId.value = productId.value + params.id + ";";
				}
				if(window.IframeCreateOrderProduct){
					window.IframeCreateOrderProduct(params, iBlockId);
				}
			}
		},
		...
		addProductSearch: function(params)
		{
			...
			var popup = new BX.CDialog({
				...
				'&LID='+(params.siteId != null ? params.siteId : this._settings.siteId) +
				'&caller=order_edit'+
				'&func_name='+funcName+
				'&STORE_FROM_ID=0'+
				'&public_mode=Y',
