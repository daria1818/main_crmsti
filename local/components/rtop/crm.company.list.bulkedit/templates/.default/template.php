...
$APPLICATION->IncludeComponent(
    ...
    'title',
    array(
        'TOOLBAR_ID' => $arResult['GRID_ID'] . '_btn',
...
?>
<form method="POST" action="/local/components/rtop/crm.company.list.bulkedit/importbulk.ajax.php" name="importBulk" style="opacity: 0;visibility: hidden;">
<input type="file" name="importCompanyCSV">
...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		'FILTER_PARAMS' => array(
			'LAZY_LOAD' => array(
				'GET_LIST' => '/local/components/rtop/crm.company.list.bulkedit/filter.ajax.php?action=list&filter_id='.urlencode($arResult['GRID_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
				'GET_FIELD' => '/local/components/rtop/crm.company.list.bulkedit/filter.ajax.php?action=field&filter_id='.urlencode($arResult['GRID_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
			)
		),
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'serviceUrl' => '/local/components/rtop/crm.company.list.bulkedit/list.ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get(),
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			var mgr = BX.CrmDuplicateManager.create("mgr", { entityTypeName: "<?=CUtil::JSEscape(CCrmOwnerType::CompanyName)?>", serviceUrl: "<?=SITE_DIR?>local/components/rtop/crm.company.list.bulkedit/list.ajax.php?&<?=bitrix_sessid_get()?>" });
			BX.addCustomEvent(
				mgr,
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildCompanySearch",
					{
						serviceUrl: "<?='/local/components/rtop/crm.company.list.bulkedit/list.ajax.php?'.bitrix_sessid_get()?>",
		...
		function()
		{
			...
			var converter = BX.CrmRequisiteConverter.create(
				"converter",
				{
					...
					serviceUrl: "<?=SITE_DIR?>local/components/rtop/crm.company.list.bulkedit/list.ajax.php?&<?=bitrix_sessid_get()?>"
				}
			);
...
?>
<script>
	var inputImportCSV = document.querySelector('[name="importCompanyCSV"]');
    function importCompanyCSV(){
    	inputImportCSV.click();
    }
    BX.bind(inputImportCSV, "change", BX.delegate(function(e){
    		...
    		bxFormData.send(
    			...
    			function(data){
    				...
    				if(data.diff.length > 0){
    					var popup = BX.PopupWindowManager.create('company-csv-list', null, {
							autoHide: false,
							offsetLeft: 0,
							offsetTop: 0,
							overlay: true,
							...
							closeIcon: true,
							contentColor: 'white',
							className: 'company-csv-list'
						});
						let elements = BX.create('DIV');
						for(let value of data.diff){
							let element = BX.create('P', {
								text: value
							});
							...
							children: [
								...
								BX.create('DIV', {
									...
									children: [
										BX.create('A', {
											attrs: {
												class: 'ui-btn ui-btn-success btn btn-success',
												href: 'javascript:void(0)'
											},
											text: data.url == '' ? 'Закрыть' : 'Перейти к списку',
											events: {
												click: function(){
													if(data.url == '')
														popup.close();
						...
						popup.setTitleBar('Непринятые строки');
