...
$arResult['GRID_ID'] = 'CRM_CONTACT_LIST_ORDER_V12'.($bInternal && !empty($arParams['GRID_ID_SUFFIX']) ? '_'.$arParams['GRID_ID_SUFFIX'] : '');
...
if (!$bInternal)
{
	...
	$arResult['FILTER'][] = [
		'id' => 'DATE_ORDER', 'name' => GetMessage('CRM_FILTER_FIELD_DATE_ORDER'), 'type' => 'date', 'selected' => true, 'default' => true, 'exclude' => array(
			DateType::NONE,
...
if(!empty($arFilter['DATE_ORDER_from']) || !empty($arFilter['DATE_ORDER_to'])){
	$arOrderFilter = [
		'>=DATE_INSERT' => $arFilter['DATE_ORDER_from'],
		'<=DATE_INSERT' => $arFilter['DATE_ORDER_to'],
		'CONTACT_COMPANY.ENTITY_TYPE_ID' => \CCrmOwnerType::Contact,
	];
	unset($arFilter['DATE_ORDER_from']);
	unset($arFilter['DATE_ORDER_to']);
}
$orders = \Bitrix\Crm\Order\Order::getList([
	'order' => array("ID" => "ASC"),
	'filter' => $arOrderFilter ?: [],
	'select' => ['ID', 'CONTACT_COMPANY_ID' => 'CONTACT_COMPANY.ENTITY_ID'],
	'runtime' => [
        'CONTACT_COMPANY' => [
...
$customEntityID = array_column($orders, 'CONTACT_COMPANY_ID');
if(isset($arFilter['ID'])){
	if(!in_array($arFilter['ID'], $customEntityID))
	{
		$arFilter['ID'] = 0;
	}
}else{
	$arFilter['ID'] = $customEntityID;
}
