...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		'FILTER_PARAMS' => array(
			'LAZY_LOAD' => array(
				'GET_LIST' => '/local/components/rtop/crm.contact.list.order/filter.ajax.php?action=list&filter_id='.urlencode($arResult['GRID_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
				'GET_FIELD' => '/local/components/rtop/crm.contact.list.order/filter.ajax.php?action=field&filter_id='.urlencode($arResult['GRID_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
			)
		),
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'serviceUrl' => '/local/components/rtop/crm.contact.list.order/list.ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get(),
...
BX.ready(
	function()
	{
		...
		var mgr = BX.CrmDuplicateManager.create("mgr", { entityTypeName: "<?=CUtil::JSEscape(CCrmOwnerType::ContactName)?>", serviceUrl: "<?=SITE_DIR?>local/components/rtop/crm.contact.list.order/list.ajax.php?&<?=bitrix_sessid_get()?>" });
		BX.addCustomEvent(
			mgr,
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildContactSearch",
					{
						serviceUrl: "<?='/local/components/rtop/crm.contact.list.order/list.ajax.php?'.bitrix_sessid_get()?>",
	...
	function()
	{
		...
		var converter = BX.CrmRequisiteConverter.create(
			"converter",
			{
				...
				serviceUrl: "<?=SITE_DIR?>local/components/rtop/crm.contact.list.order/list.ajax.php?&<?=bitrix_sessid_get()?>"
			}
		);
