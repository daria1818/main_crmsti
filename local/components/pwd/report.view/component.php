...
try
{
	...
	switch ($period['type'])
	{
		...
		case 'interval':
			[$date_from, $date_to] = $period['value'];
    ...
    if($report['DESCRIPTION'] === 'REPORT_BAR'){
        $filter[] = [
            'LOGIC' => 'OR',
            '!DEAL_OWNER.COMPANY_BY.ID' => false,
            '!DEAL_OWNER.CONTACT_BY.ID' => false
        ];

        foreach ($settings['select'] as &$itemSelect){
            if($itemSelect['name'] == 'DEAL_OWNER.CONTACT_BY.ID'){
                $itemSelect['grouping'] = true;
            }
        }
        unset($itemSelect);
        $uniqueGrid = true;
        ...
        $setBar = (int) $request->getQuery('BAR');
        ...
        while($one = $bars->GetNext()){
            ...
            $arResult['BAR'][] = [
                'name' => CatalogTable::getEnumValueById($idProp, 'PRACTICE'),
                'id' => $one['ID'],
                'selected' => $setBar === (int)$one['ID']
            ];
        }
	...
	foreach ($settings['select'] as $num => $elem)
	{
		...
		[$alias, $selElem, $totalInfo] = CReport::prepareSelectViewElement(
	...
	{
	    $hash = $row['CRM_PRODUCT_ROW_DEAL_OWNER_COMPANY_BY_ID'].'_'.$row['CRM_PRODUCT_ROW_DEAL_OWNER_CONTACT_BY_ID'];
	    if(
	        $uniqueGrid &&
            in_array($hash, $dataIDs)
        ){
	        continue;
        }
        $dataIDs[] = $hash;
	...
	if ($need_concat_rows && $grcDataPrimaryValues)
	{
		...
		foreach ($grcSettingsNum as $num)
		{
			...
			[$alias, $selElem] = CReport::prepareSelectViewElement(
				$elem, $settings['select'], $is_init_entity_aggregated, $fList, $fChainList,
				$arParams['REPORT_HELPER_CLASS'], $entity
			);
