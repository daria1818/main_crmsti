...
final class AjaxProcessor extends \Bitrix\Crm\Order\AjaxProcessor
{
    ...
    public static $replaceCode = [
        ...
        'COMPANY_TITLE' => 'RQ_COMPANY_NAME',
        'PHONE' => 'PHONE_WORK',
        'EMAIL' => 'EMAIL_WORK',
        'INN' => 'RQ_INN',
        'COMPANY_RQ_BIK_1' => 'RQ_BIK',
        'COMPANY_RQ_KPP_1' => 'RQ_KPP',
        'COMPANY_RQ_OGRN_1' => 'RQ_OGRN',
        'KA' => 'RQ_COR_ACC_NUM',
        'COMPANY_RQ_BANK_NAME_1' => 'RQ_BANK_NAME',
        'COMPANY_RQ_COR_ACC_NUM_1' => 'RQ_COR_ACC_NUM',
        'COMPANY_RQ_ACC_NUM_1' => 'RQ_ACC_NUM',
    ];
	...
	protected function formatResultData(Order $order, array $changedFields = array())
	{
        ...
        self::modificationOrderFields($freshData);
		$freshData['OLD_USER_PROFILE'] = $freshData['USER_PROFILE'];
		return $freshData;
	}


    protected static function modificationOrderFields(&$freshData)
    {
        if ($freshData['COMPANY_ID']) {

            self::getAddressCompany($freshData['COMPANY_ID'], $freshData);

            $props = OrderPropsTable::getList([
                'filter' => ['CODE' => array_keys(self::$replaceCode)],
            ])->fetchAll();

            foreach ($props as $prop) {
                if (
                    empty($freshData['PROPERTY_' . $prop['ID']]) &&
                    self::$replaceCode[$prop['CODE']]
                ) {
                    $freshData['PROPERTY_' . $prop['ID']] = self::$replaceCode[$prop['CODE']];
                }
            }
        }
    }
    ...
    public static function getAddressCompany(?int $companyID, ?array $freshData): void
    {
        ...
        $entityCompanyID = $entityCompany['ID'];
        $fieldData = [];
        // Реквизиты
        $bankData = (new EntityBankDetail())->getList([
            "filter" => [
        ...
        if(is_array($bankData)){
            $fieldData = $bankData + $entityCompany;
        }

        // Контакты
        foreach ($freshData['MULTIFIELD_DATA'] ?: [] as $code => $field) {
            $data = current(current($field));
            if ($data['VALUE']) {
                $fieldData[$data['COMPLEX_ID']] = $data['VALUE'];
            }
        }

        foreach (self::$replaceCode as $code => &$replaceField) {
            if (gettype($replaceField) !== 'string') {
                continue;
            }
            $replaceField = $fieldData[$replaceField] ?: '';
            unset($replaceField);
        }
