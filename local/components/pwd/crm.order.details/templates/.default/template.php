...
foreach ($arResult['ENTITY_CONTROLLERS'] as &$controller) {
    if ($controller['config']['serviceUrl'] == '/bitrix/components/bitrix/crm.order.details/ajax.php') {
        $controller['config']['serviceUrl'] = '/local/components/pwd/crm.order.details/ajax.php';
    }
    unset($controller);
}
...
$APPLICATION->IncludeComponent(
    ...
    [
        ...
        'TABS' => $arResult['TABS'],
        'SERVICE_URL' => '/local/components/pwd/crm.order.details/ajax.php?action=convert&' . bitrix_sessid_get(),
        'REST_USE' => 'Y',
        'EDITOR' => [
            'GUID' => "{$guid}_editor",
            ...
            'SERVICE_URL' => '/local/components/pwd/crm.order.details/ajax.php?' . bitrix_sessid_get(),
